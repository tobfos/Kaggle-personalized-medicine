...
def create_submission(hyperparams, name_string, model_setup={}):
    ...
    nn_simple.fit(train.iloc[:, :-1], np.array(train.iloc[:, -1]).reshape((-1, 1)))
...
def create_submission_normalized_noise(hyperparams, name_string, model_setup={}):
    nn_simple = MyNeuralNets(input_dim=train.shape[1]-1,
                            n_classes=len(np.unique(train['Class'])),
                            hyperparams=hyperparams,
                            model_setup=model_setup)
...
def cross_validate_noise(hyperparams, cur_model):
    nn_simple = MyNeuralNets(input_dim=train.shape[1]-1,
                            n_classes=len(np.unique(train['Class'])),
                            hyperparams=hyperparams,
                            model_setup=cur_model)
    scores = nn_simple.cross_validate(train_noise.iloc[:, :-1], train_noise.iloc[:, -1], cv=3, architecture_type='dynamic')
    return scores
...
def cross_validate_normalized_noise(hyperparams, cur_model):
    nn_simple = MyNeuralNets(input_dim=train.shape[1]-1,
                            n_classes=len(np.unique(train['Class'])),
                            hyperparams=hyperparams,
                            model_setup=cur_model)
    scores = nn_simple.cross_validate(train_normalized_noise.iloc[:, :-1], train_normalized_noise.iloc[:, -1], cv=3, architecture_type='dynamic')
    return scores

train = pd.read_csv('data/train_1.csv', index_col=0)
test = pd.read_csv('data/test_1.csv', index_col=0)

#train.iloc[:, :-1] += np.random.normal(0, scale=0.08, size=(train.shape[0], train.shape[1]-1))
train_noise = train.copy()
train_noise.iloc[:, :-1]  += np.random.normal(0, scale=0.08, size=(train.shape[0], train.shape[1]-1))
...
train_normalized_noise = train_noise.copy()
scaler_2.fit(train_normalized_noise.iloc[:, :-1])
train_normalized_noise.iloc[:, :-1] = scaler_2.transform(train_normalized_noise.iloc[:, :-1])
...
adam_3 = {'loss': 'categorical_crossentropy',
            'optimizer': optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=10**-3),
            'dense_layers': [5000, 5000, 1200, 64, 16],
            'activation': ['relu', 'relu', 'relu', 'relu', 'relu'],
            'dropout': [0.5, 0.5, 0.2, 0.1, 0],
            'regularizer': [regularizers.l2(0.2), regularizers.l2(0.2), regularizers.l2(0.05), regularizers.l2(0.05), regularizers.l2(0.05)],
            'batchnormalization': [1, 1, 1, 0, 0]}

adam_2 = {'loss': 'categorical_crossentropy',
            'optimizer': optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=10**-3),
            'dense_layers': [5000, 2500, 32, 16],
            'activation': ['relu', 'relu', 'relu', 'relu'],
            'dropout': [0.5, 0.5, 0.1, 0],
            'regularizer': [regularizers.l2(0.2), regularizers.l2(0.2), regularizers.l2(0.05), regularizers.l2(0.05)],
            'batchnormalization': [1, 1, 1, 0]}
...
create_submission(hyperparams, 'test_30_aug_2', adam_3)
#create_submission_normalized(hyperparams, 'test_day_2_normalized', adam_3)

#sampled_rates = np.random.uniform(-5, 0, 5)

model_list = [adam_2, adam_3]
model_name_list = ['Adam 2', 'Adam 3']
epochs_list = [100]
batch_size_list = [512]
#epochs_list = []
#batch_size_list = []
...
for model_n, model in enumerate(model_list):
    for epoch in epochs_list:
        for batch_size in batch_size_list:
            ...
            cur_model = model
            scores = cross_validate(hyperparams, cur_model)
